...
def build_index() -> Path:
  ...
  if (reports_root / "latest-report.html").exists():
    latest_link = (
      '<p class="tip"><strong>最新报告：</strong>'
      '<a href="latest-report.html" target="_blank">reports/latest-report.html</a>'
      "（双击即可打开）</p>"
    )
...
<body>
  <h1>熊猫掌柜收银台 UI 自动化 - 历史测试报告</h1>
  {latest_link}
  <p class="tip">每次执行后会归档到 <code>reports/runs/运行编号/</code>，可直接双击 HTML 查看。</p>
  <table>
    <thead>
      <tr>
        <th>运行编号</th>
    ...
    <tbody>
      {''.join(rows) if rows else '<tr><td colspan="7">暂无历史记录，请先双击 run_tests_and_report.bat</td></tr>'}
    </tbody>
  </table>
